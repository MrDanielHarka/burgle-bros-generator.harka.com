...
var Burgle = (function () {
    ...
    for (var f = 0; f < floors; f++) {
      ...
      if (window.location.href.indexOf('/hu') == -1) {
        if (size == 7) {
          floor.setAttribute(
            'class',
            (size == 7 ? 'harka' : 'bank') + ' floor'
          );
          document.getElementById('generate-all-floors').textContent =
            'Generate floor';
        } else {
          document.getElementById('generate-all-floors').textContent =
            'Generate All floors';
      ...
      if (window.location.href.indexOf('/hu') != -1) {
        btn.textContent = f + 1 + '. szint generálása';
      }
      ...
      if (size == 7) {
        document.querySelector('button.center').style.display = 'none';
      }
    }
  };
  ...
  var update_href = function () {
    ...
    history.replaceState(link, 'Title of the page', link);
    ...
    document.getElementById(
      'share-pinterest'
    ).href = `https://pinterest.com/pin/create/button/?url=${link}&media=https://burgle-bros-generator.harka.com/img/burgle-bros-generator.png`;
    document.getElementById('share-email').href = `mailto:?body=${link}`;

    const urlCopyButton = document.getElementById('burgle_href');

    urlCopyButton.addEventListener('click', () => {
      navigator.clipboard.writeText(link);

      if (window.location.href.indexOf('/hu') == -1) {
        urlCopyButton.textContent = 'Copy done';
        setTimeout(() => {
          urlCopyButton.textContent = 'Copy layout link';
        }, 1000);
      } else {
        urlCopyButton.textContent = 'A link másolása elkészült';
        setTimeout(() => {
          urlCopyButton.textContent = 'Elrendezés linkjének másolása';
        }, 1000);
      }
    });
  };
  ...
  var generate = function (id) {
    ...
    if (id === undefined || id === 'all') {
      ...
      if (size === 7) {
        ...
        //   bankTds = document.querySelectorAll('.bank td'),
        //   bankTdTiles = document.querySelectorAll('.bank td.tile');
        // console.log(bank, bankTds, bankTdTiles);
        // bank.classList.add('small');
        // for (var bankTd of bankTds) {
        //   bankTd.classList.add('small');
        // }
        // for (var bankTdTile of bankTdTiles) {
        //   bankTdTile.classList.add('small');
        // }
      }
    } else {
...
languagePicker.addEventListener('change', () => {
  var languageCode = languagePicker.value;
  ...
  if (languageCode == 'en') {
    ...
    location.href =
      'https://burgle--bros--generator-harka-com.translate.goog/?_x_tr_sch=http&_x_tr_sl=en&_x_tr_tl=' +
      languageCode;
  }
});
